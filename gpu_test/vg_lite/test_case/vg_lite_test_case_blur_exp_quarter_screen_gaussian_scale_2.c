...
static vg_lite_error_t on_setup(struct vg_lite_test_context_s* ctx)
{
    ...
    vg_lite_buffer_t* image = vg_lite_test_context_alloc_src_buffer(
        ctx,
        GPU_ALIGN_UP(target_buffer->width / 2, 16),
        GPU_ALIGN_UP(target_buffer->height / 2, 16),
        VG_LITE_BGRA8888,
        VG_LITE_TEST_STRIDE_AUTO);

    vg_lite_rectangle_t rect = { 0, 0, target_buffer->width / 2, target_buffer->height / 2 };
...
VG_LITE_TEST_CASE_ITEM_DEF(blur_exp_quarter_screen_gaussian_scale_2, GAUSSIAN_BLUR, "Test exp blur (radius=5) quarter screen and use gaussian filter and scale 2.0");
